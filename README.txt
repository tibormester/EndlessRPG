v1.1.1
New Design Ideas: Mordeuh like physics based combat but with a lot of abilities. Abilities will no longer be completely modular and editable like I wanted since it just doesn’t make sense and would be too complex to be fun. Instead utility abilities will be constant, movement abilities will have stat scalings, and different varieties of combat abilities will have different flavours of progression: fighting styles will have combo abilities where sequential attacks have increased stats and the player can change the requirements to maintain the combo or what stat multipliers they want, or for a mage like character, there will be the ability to alter the alchemical spell pathway if I ever get around to implementinng factory meets magic expansion…

Okay, so now that is out of the way how abilities will work, onto my frustrations for today, the reason I had to change abilities: root motion is too tricky in godot with procedural animation. I was trying to think of a way where walking was a sequence of stepping actions and the user input somehow got translated to targeting the feet into the right places to make it feel smooth etc… and its a dumb idea. Maybe cool for a walking simulator game, but for smooth fun combat, I need the core mechanics to feel good and smooth with plenty QoL. Realistically that can only be done by hardcoding a lot of exceptions and edge case checking and stuff into these abilities to smooth out the experience. Also I spent a good amount of time trying to get the visual aspect of the game to work and I think I am going to run into an issue with generating collision shapes for my limbs if I use godot nodes because I don’t think it can tell which specific collision shape is hit just the object… anyways so i rigged up a staticbody per body part and they all assembled together and i could adjust the bodies size and limb length either proportionally or individually. This was all well and good except its a waste of time because there almost certainly a better way to get this skeleton like behavior… maybe using a skeleton?! So, I am just gonna postpone and work on the model itself in C and try and get the backend running where it’ll just pretend its generating and simulating the world… the issue will be that without the visuals theres no collision detection so I wont be able to test anything… But honestly thats fine because I can just get the class frames built and hash out how everything is going to work and interact in theory and then when i come back to test it, the second iteration will be much better!
